...
def multi_test(future_code, begin, end, strategy, strat_params, stop_strat, stop_loss=3, slippage=2, double_side=True,
               pic=False):
    """
    ...
    for code in future_code:
        # 获取信号
        strat_signal, stop_signal, position = frame_block.get_position(np.array(data[code].Close), strategy,
            strat_params, stop_strat, stop_loss, double_side)
        # 考虑滑点，计算调整收益率
...
def cross_trade_test(future_code, begin, end, strategy, strat_params, stop_strat, stop_loss=3, slippage=2,
    ...
    # 回测
    strat_signal, stop_signal, position = frame_block.get_position(spread, strategy, strat_params, stop_strat, 3,
        double_side)
...
if __name__ == '__main__':  # 测试
    ...
    cross_trade_test(['CU_LME', 'CU', 'USDCNY'], '2010-01-01', '2016-07-19', 'Mean_Reversion', (20, 2, -2), 'percent', 3, double_side=True, pic=False)
